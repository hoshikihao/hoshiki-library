...
- `tags` 必填,字符串数组
- `date` 必填,收录时间,ISO 8601 带 `+08:00`(如 `"2026-09-08T22:30:00+08:00"`,精确到分即可),非原文发表日期;列表页与 RSS 按它倒序,同一天多篇靠时间分先后;页面上只显示到「日」(`src/lib/date.ts` 的 `ymd()` 固定按东八区格式化,不要用 `toISOString`)
- `written` 可选,原文写作时间,自由文本(如「北宋嘉佑二年（1057）」);填了就替代文章页 byline 里的日期行。没填时:文章页新枝显示「写于 <date>」(原创随笔,`date` 即写作日),古木/星雨显示「收录于 <date>」;列表卡片一律只显示日期,无前缀。纯数字年份(如 `2025`)必须加引号写成 `"2025"`,否则 YAML 解析成 number 导致 build 报错
- `sourceUrl`:可选,有权威在线原文时填(渲染成文末「阅读原文」按钮);星树浩自己写的、以及书籍/文学节选(仅有个人转录页的)不填,用 `written: 选自《书名》`
- `author` 可选
- `commentary` 可选,Markdown,渲染到文末点评块
- `related` 可选,相关阅读,`{ title, url, source? }` 数组,渲染到文末链接列表
- `draft` 可选,默认 `false`
- 正文文件只放原文(英文源文章为中英对照),不写点评、不写摘要

## 双语对照(英文源文章)

- 英文原文的文章,正文做中英对照:逐段先英文原文,紧接中文译文,交替到全文结束
- 配图、引文块置于对应段落位置;原文有图注的,英文、中文各一行
- 标题在 frontmatter 用中文;正文首行用斜体放英文原标题
- `written` 填原文写作时间,`sourceUrl` 填原文链接
- 非英文源(中文原创、已是中文的转载)照常只放中文
...
1. 网络搜索全文
2. 找不到:查 Wayback Machine 存档
3. 用户直接提供文件(epub/txt/html 等)
4. 仍找不到:告知用户该篇出自哪本书,由用户去 Z-Library / Anna's Archive 下载电子书,再从中提取正文
...
- 加书:先把候选封面图发给用户确认,再在 `src/data/books.yaml` 追加一条
- 加片:先把候选海报图发给用户确认,再在 `src/data/films.yaml` 追加一条
